...
src_install() {
	...
	# User-facing CLIs
	dobin configure.py configure.sh bin/kb-brightness bin/kb-platform-profile bin/platform-fan bin/platform-probe bin/platform-power bin/platform-touchpad bin/platform-metrics bin/platform-screen-swap
	dobin bin/kb-fan
	if use fan_control; then
	...
	# Live typing-inhibit filter (OpenRC / systemd); same pidfile as GUI
	newinitd contrib/openrc/zenbook-platform-touchpad zenbook-platform-touchpad
	newconfd contrib/openrc/conf.d/zenbook-platform-touchpad zenbook-platform-touchpad
	insinto /etc/systemd/system
	doins contrib/systemd/zenbook-platform-touchpad.service
	...
	for doc in LICENSE DEPLOY.md README.ux8406.md README.ux5400.md \
		README.fn_row_policy.md CHANGELOG.md \
...
pkg_postinst() {
	...
	elog "Touchpad live filter (typing-inhibit):"
	elog "  edit /etc/zenbook-scripts/touchpad.json (from .example)"
	elog "  rc-update add zenbook-platform-touchpad default   # optional"
	elog "  or: platform-touchpad-gui / platform-touchpad run"
